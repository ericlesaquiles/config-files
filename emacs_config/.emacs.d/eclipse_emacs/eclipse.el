...
;; Copyright (C) 1986, 1987, 2001 - 2016
...
;; eclipse-libdirs  List of strings that contain the paths to the ECLiPSe
;;     library directories. When loading project files, these directories
;;     will be searched for source code files of modules loaded by lib/1 calls.
...
;; This version has been tested on emacs 24.3.1 for Linux
...
;; The code changes in the inferior mode and the handling of newline have
;; largely been retracted again, as comint cannot handle the complex behaviour
;; of ECLiPSe wrt newline

;; Added variable eclipse-host, which can be used to override the hostname that
;;  attach_tools/3 supplies.
;;  E.g., tktools cannot handle a hostname like "<name>.site", which might be a problem
;;  on some non-networked Linux systems.
...
(defconst eclipse-tktools-attach-call
...
(defcustom eclipse-libdirs '()
  "List of recognized ECLiPSe library directories."
  :type 'list
  :group 'eclipse)

(defcustom eclipse-host ""
  "Name of ECLiPSe host for TkTools."
  :type 'string
  :group 'eclipse)
...
(defconst eclipse-font-lock-keywords-0
  eclipse-font-lock-basic
  "Even more basic (Prolog) expressions for font-lock mode.")
...
(defconst eclipse-font-lock-keywords
	(cond ((= eclipse-font-lock-default -1) nil)
				((= eclipse-font-lock-default 0) eclipse-font-lock-keywords-0)
...
Contains commands that are associated with editing an ECLiPSe file."
	'("ECLiPSe-Edit"
		...
		("Check"
		 ...
		 "--"
		 ["Check buffer with lint library" eclipse-lint-buffer (not eclipse-esp-selected)])
		("Outline"
		 ("Hide"
     ...
     "--"
     ("Font Lock"
      ["Font Lock Off" eclipse-font-lock-off t]
      ["Font Lock Level 0" eclipse-font-lock-0 t]
...
The fontification can be set to five different levels:
\\[eclipse-font-lock-0], \\[eclipse-font-lock-1], \\[eclipse-font-lock-2], \\[eclipse-font-lock-3],
\\[eclipse-font-lock-4], or shut off using \\[eclipse-font-lock-off]. When ESP files are edited,
the editor automatically switches to a different fintification scheme, which also can be manually
switched on using \\[eclipse-font-lock-5].
...
(defun eclipse-font-lock-0 ()
  "Switch font lock to primitive level."
  (interactive)
  (eclipse-do-font-lock eclipse-font-lock-keywords-0))
...
If auto-indent is on, the next line is automatically indented."
  ...
  (interactive)
  (let (string (flag nil) proc beg aux end len (name (buffer-name)))
    (cond ((string-equal name eclipse-process-buffer)
					 ...
					 (cond
						...
						((save-excursion
						 ...
						 (comint-send-string (get-process eclipse-process-name) (concat string "\n"))
						...
						((save-excursion
						 ...
						 (cond ((eq beg (point))
										...
										(eclipse-set-process-mark)
										(comint-send-string (get-process eclipse-process-name) "\x0d"))
									 (t
										...
										(insert "\n")
										(eclipse-set-process-mark)
										(comint-send-string (get-process eclipse-process-name) string))))
						...
						((save-excursion
						 ...
						 (setq len (- beg (point)))
						 (cond ((eq len 0)  ;; don't know, why (eq beg (point)) doesn't work as well?!?
										;; if empty input, send CR to eclipse
										(eclipse-set-process-mark)
										(comint-send-string (get-process eclipse-process-name) "\n"))
									 (t
										;; else fontify and send command to eclipse
										(setq string (buffer-substring beg (point)))
										(insert "\n")
...
call."
  ...
  (unless eclipse-esp-selected
    (let* ((blist (if flag (buffer-list) ; start with current buffer
					 ...
					 (all-buffers blist)
					 name ext begin end filename command buffer)
      (save-excursion
				(while (car blist) ; iterate through all buffers
					...
					(when (member ext eclipse-extensions)
						...
						(goto-char (point-min))
						(while (not (eobp))
							(cond ((re-search-forward "\\<\\(use_module\\|ensure_loaded\\|compile\\|include\\|lib\\)('?\\([^')]+\\)'?)"
																				(point-max) t)
										 (setq filename (match-string 2))
										 (setq command (match-string 1))
										 (if (string-equal command "lib")
												 (let* ((d nil) (fn nil)
																(libdirs eclipse-libdirs))
													 (while (and libdirs (not fn))
														 (setq d (car libdirs)
																	 libdirs (cdr libdirs))
														 (let ((exts eclipse-extensions))
															 (while (and exts (not fn))
																 (setq fn (concat d "/" filename (car exts))
																			 exts (cdr exts))
...
;;;;;;
;; The following two functions are copied from cmuscheme.el.
(defun eclipse-proc ()
  "Return the current ECLiPSe process, starting one if necessary.
See variable `eclipse-buffer'."
  (unless (and eclipse-buffer
               (get-buffer eclipse-buffer)
               (comint-check-proc eclipse-buffer))
    (run-eclipse))
  (or (eclipse-get-process)
      (error "No ECLiPSe process is running.")))

(defun eclipse-get-process ()
  "Return the current ECLiPSe process or nil if none is running."
  (get-buffer-process (if (eq major-mode 'inferior-eclipse-mode)
                          (current-buffer)
                        eclipse-buffer)))
  ...
  (let (eclipse-window)
    ...
    (save-excursion
      ...
      (switch-to-buffer (make-comint eclipse-process-name eclipse-program-call))
			(setq eclipse-buffer eclipse-process-buffer)
      (inferior-eclipse-mode))))
  ...
  (let (eclipse-window eclipse-status)
    (save-excursion
      ...
      (cond ((not eclipse-status)
						...
						(t
						 (comint-send-string (eclipse-proc) "\C-c")
  ...
  (let (eclipse-window eclipse-status)
    (save-excursion
      (setq eclipse-window (get-buffer-window eclipse-process-buffer)
						eclipse-status (process-status eclipse-process-name))
      (when eclipse-status
				(comint-send-string (eclipse-proc) eclipse-halt-string))
      (mapcar '(lambda (buf)
    ...
    (save-excursion
      ...
      (cond ((not eclipse-status)
						...
						(t
						 ...
						 (insert eclipse-tktools-attach-call)
						 (eclipse-set-process-mark)
						 (process-send-string eclipse-process-name eclipse-tktools-attach-call))))))
...
(defun eclipse-run-tktools (output)
  ...
  (let (host port output-list list head tail)
    (cond ((string-match "\\[.*, .*\\]\n" output)
					 ...
					 (if (not (string-equal eclipse-host ""))
							 (setq host eclipse-host)
						 (setq host (nth 1 list)))
					 (setq port (nth 2 list))
...
(defun eclipse-compile-region (compile beg end)
  ...
  (unless eclipse-esp-selected
    (save-excursion
      (add-hook 'comint-preoutput-filter-functions 'eclipse-remove-empty-lines)
      (comint-send-string (eclipse-proc) eclipse-compile-string)
...
(defun eclipse-toggle-source-bp1 (&optional output)
  ...
  (when (or (not eclipse-dbgflag)
    ...
    (let* ((line (line-number-at-pos))
      ...
      (comint-send-string (eclipse-proc) str))
...
(defun eclipse-toggle-source-bp2 (output)
  ...
  (when (string-match "\\[eclipse [0-9]+\\]:" output)
    ...
    ;; leave break level
    (comint-send-string (eclipse-proc) "end_of_file.\n"))
  output)
  ...
  (when eclipse-source-tracing
    ...
    (cond ((and (equal (buffer-name) eclipse-process-buffer)
					...
					((and (get-buffer eclipse-process-buffer)
					 ...
					 (comint-send-string (eclipse-proc) "b\x0d"))
					(t
...
(defconst eclipse-font-lock-compilation
  (list
   '("\\*\\*\\* \\(WARNING: [^*]+\\) \\*\\* \\([^*]+\\) \\*\\* \\([0-9]+\\) \\*\\* \\(-?[0-9]+\\)" (1 font-lock-warning-face) (2 font-lock-function-name-face) (3 font-lock-builtin-face) (4 font-lock-builtin-face))
   '("\\*\\*\\* \\([^*]+\\) \\*\\* \\([^*]+\\) \\*\\* \\([0-9]+\\) \\*\\* \\(-?[0-9]+\\)" (1 font-lock-warning-face) (2 font-lock-function-name-face) (3 font-lock-builtin-face) (4 font-lock-builtin-face))
   '("\\*\\*\\* \\([^*]+\\) \\*\\* \\([^*]+\\) in module \\(.+\\)" (1 font-lock-warning-face) (2 font-lock-builtin-face) (3 font-lock-function-name-face))
   '("\\(WARNING: Directive failed or aborted in file\\) \\([^,]+\\), line \\([0-9]+\\):" (1 font-lock-warning-face) (2 font-lock-function-name-face) (3 font-lock-builtin-face))))

(defun insert-file-link (name line)
  ;; insert name and make link
  (let (pnt call
						(map (make-sparse-keymap)))
    (setq pnt (point))
    (insert name)
    (make-text-button pnt (point)
											'action 'eclipse-follow-file-link
											'follow-link t
											'filename name
											'fileline line)))

(defun eclipse-follow-file-link (button)
  ;; follow link
  (let ((name (button-get button 'filename))
				(line (string-to-number (button-get button 'fileline)))
				(ecb (get-buffer eclipse-compilation-buffer)))
    (if (window-live-p (get-buffer-window ecb))
				(progn
...
(defun parse-and-insert (messages)
  ...
  (let ((curr-file nil)
				(curr-buffer nil)
				(curr-offset nil)
				(curr-line-offset nil)
				start end aux line col clause predicate msg pnt filename)
    ;; Parse messages.
    (while messages
      ;; next message
      ...
      (when (and msg (not (equal msg "")))
				(cond ((equal (substring msg 0 13) "*** predicate") ; missing predicate
							 (setq start (+ 10 (string-match "in module " msg))
										 aux (substring msg start (length msg))
										 msg (substring msg 0 start))
							 (cond ((get-buffer (concat aux ".ecl"))
											(setq aux (concat aux ".ecl")))
										 ((get-buffer (concat aux ".eco"))
											(setq aux (concat aux ".eco")))
										 ((get-buffer (concat aux ".pl"))
											(setq aux (concat aux ".pl")))
										 (t (setq aux nil)))
							 (cond (aux
											(insert msg)
											(insert-file-link aux "1")
											(insert " ** 1 ** 1"))
										 (t (insert msg))))
							((equal (substring msg 0 3) "***") ; error or warning
							 (setq col (string-to-number (cadr (cddr (split-string msg "\\( \\*\\* \\)"))))
										 aux (eclipse-compile-find-pos (car curr-buffer) (+ col (car curr-offset)))
										 line (car aux)
										 col (cadr aux)
										 aux (split-string msg "\\( \\*\\* \\)")
										 msg (concat (car aux) " ** "))
							 (insert msg)
							 (insert-file-link (cadr aux) line)
							 (setq msg (concat " ** " line " ** " col))
							 (insert msg))
							 ...
							 (setq aux (split-string msg "\\(: \\)")
										 ...
										 line (number-to-string
													 (- (string-to-number (cadr (split-string (car aux) "\\(line \\)")))
															(if (string-match "File user" (car aux)) (- 86 (car curr-line-offset)) 0)))
										 msg (concat "*** WARNING: " msg " ** "))
							 (insert msg)
							 (insert-file-link (car curr-file) line)
							 (setq msg (concat " ** " line " ** 1"))
							 (insert msg))
							((equal (substring msg 0 3) ">>>") ; new file opened
							 ...
							 (cond ((string-equal filename "user")
											...
											(setq curr-file (cons (buffer-file-name (get-buffer eclipse-compile-buffer)) curr-file)
														curr-buffer (cons eclipse-compile-buffer curr-buffer)
														curr-offset (cons eclipse-compile-offset curr-offset)
														curr-line-offset (cons (save-excursion
										 ...
										 (t ; else get filename and open file if necessary, put info on stack
											(setq curr-file (cons filename curr-file)
														curr-buffer (cons (buffer-name (find-file-noselect filename t)) curr-buffer)
														curr-offset (cons 0 curr-offset)
														curr-line-offset (cons 0 curr-line-offset))))
							 (insert msg))
							((equal (substring msg 0 3) "<<<") ; file finished: pop stack
							 (setq curr-file (cdr curr-file)
										 curr-buffer (cdr curr-buffer)
										 curr-offset (cdr curr-offset)
										 curr-line-offset (cdr curr-line-offset))
							 (insert msg))
							((string-match "structure" msg) ; unrecognized structure: we only support new style (ECLiPSe 6.0 and later)
							 ...
							 (setq msg (concat " ** " line " ** " col))
							 (insert msg))
							(t (insert msg))) ; else do nothing
				(insert "\n")))
    ;; fontify
    (let ((font-lock-verbose nil))
      (setq font-lock-defaults '(eclipse-font-lock-compilation nil)
						font-lock-keywords eclipse-font-lock-compilation)
      (font-lock-fontify-buffer))
    (set-buffer-modified-p nil)))

(defun parse-lint-and-insert (bufname messages)
  ;; parses lint warnings
  (let* ((buf (get-buffer bufname)) (compilation-error-list nil)
				 (curr-file (buffer-file-name buf))
				 msg aux aux2 line)
    ;; Parse messages.
    (while messages
      (setq msg (car messages)
      ...
      (when (and msg (not (equal msg "")))
				(cond ((or (equal (substring msg 0 4) "File")
							 ...
							 (setq line (cadr (split-string aux2 "\\(line \\)"))
										 msg (concat "*** WARNING: " msg " ** "))
							 (insert msg)
							 (insert-file-link curr-file line)
							 (setq msg (concat " ** " line " ** 1"))
							 (insert msg))
							((equal (substring msg 0 4) "WARN")
							 (setq msg (concat msg " " (car messages))
										 ...
										 line (cadr aux)
										 msg (concat "*** WARNING: " (car aux)  msg " ** "))
							 (insert msg)
							 (insert-file-link curr-file line)
							 (setq msg (concat " ** " line " ** 1"))
							 (insert msg))
...
(defun eclipse-call-compile (mode code &optional fileflag)
  ...
  (save-excursion
    ...
    (let ((ecb (get-buffer eclipse-compilation-buffer))
      ...
      (define-key map "\M-n" 'forward-button)
      (define-key map "\M-p" 'backward-button)
      (use-local-map map)
